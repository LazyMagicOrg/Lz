...
public class AwsLambdaTenantServiceComponent : ComponentResource, ITenantServiceComponent
{
        ...
        IDatabaseOutputs database, ITenantDataOutputs tenantData)
    {
        ...
        // DynamoDB — scoped to this system's tables. MUST mirror the Fargate task
        // role (AwsFargateTenantServiceComponent): in addition to the hyphenated
        ...
        // =====================================================================
        // Two gaps the Lambda role has vs Fargate that the BFF needs:
        //   1. secretsmanager:GetSecretValue on {sk}/{tk}* (Fargate already
        ...
        if (Lz.Aws.Shared.BffWiring.IsEnabled(tenantConfig))
        {
            var dpParam = Lz.Aws.Shared.BffWiring.DataProtectionParamPath(sk, env);
        ...
        // the Fargate task definition injects (AwsFargateTenantServiceComponent).
        ...
        var bffEnv = Lz.Aws.Shared.BffWiring.IsEnabled(tenantConfig)
            ? Lz.Aws.Shared.BffWiring.BuildEnv(tenantConfig, this)
